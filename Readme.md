...
**Project Summary:** This repository contains an R-based pipeline for analyzing 10x Genomics Visium spatial transcriptomics data, developed as part of a Master's thesis investigating whether microRNAs (miRNAs) regulate gene expression at cell-type boundaries. The pipeline integrates established spatial transcriptomics tools (SPATA2, Seurat, BayesSpace) with miRNA target predictions (TargetScan) to process Visium datasets, denoise gene expression via autoencoders, cluster spatial spots, analyze miRNA–target interactions, and visualize spatial expression patterns. An interactive Shiny app is included for exploring the results in a user-friendly way.
